...
function GamesHub() {
  const { user, gameProgress, settings } = useUserStore();
      ...
      <Screen>
        ...
        <div className="mt-7 flex items-baseline justify-between">
          ...
          <span className="text-xs font-bold text-muted-foreground">
            {games.filter((g) => !g.locked && !g.comingSoon).length} active quests
          </span>
        </div>
        ...
        <ul className="mt-3 grid grid-cols-2 gap-3">
          {games.map((game, i) => {
            ...
            const isUnlocked = !isComingSoon && (!game.locked || user.xp >= (game.unlockXp || 99999));
            ...
            const card = (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {isComingSoon && (
                    <span className="rounded-full bg-sun px-2 py-0.5 text-[10px] font-black text-sun-foreground shadow-xs">
                      Coming Soon
                    </span>
                  )}
                </div>
                ...
                {isUnlocked && game.levels.length > 0 && (
                ...
                {isComingSoon ? (
                  <div
                    onClick={() => {
                      ...
                      toast.info("Entrepreneurship quest is coming soon! Mini business challenges and founder pitch tools are arriving shortly.");
                    }}
